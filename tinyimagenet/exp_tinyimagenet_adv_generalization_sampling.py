...
def test_repaired(args: Namespace) -> float:
    ...
    adv_test_set = TinyImagenetPoints.load(train=False, device=device, net=args.net, repairnumber=args.repair_number, testnumber=args.test_datasize, radius=args.repair_radius,is_attack_generalization=True)
    
    logging.info(f'--evaluate the repaired net on testset and get the bitmap')
    ratio = eval_test(original_net, adv_test_set)
    logging.info(f'--For testset, out of {len(adv_test_set)} items, ratio {ratio}')  
    ...
    for images, labels in testloader:
        ...
        # ALREADY TODO: recently method: get the top2 prediction of the original net,  and 1st prediction is the correct prediction
        # 2nd prediction is the wrong prediction
        # with torch.no_grad():
        ...
        # _,index_storage = outs_max_storage.topk(2, dim=1)
        ...
        index = random_sample(sample_n=10, std=0.1, inputs=images, 
        ...
        #TODO try to use all patch, set bitmap to ones
        # repaired_net.feature_sumnet.bitmap = torch.ones(index.size(0), args.repair_number).to(device)
    ...
    # with open(Path(COMP_DIR, f'tiny_generalization.txt'), 'a') as f:
