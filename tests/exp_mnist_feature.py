...
def repair_mnist(args: Namespace, weight_clamp = False)-> Tuple[int, float, bool, float]:
    ...
    # We want to use k-means algorithm to get the center of every element of sample_datas, 
    # which is firstly dimensionality reducted by PCA. 
    # As for every dataset, and t is a theshold. 
    # If the distance of element of dateset is larger then t, we pass; 
    # else we choose the center which has minimum distance
    ...
    for i in range(len(sample_datas)):
        ...
        reduction_data = torch.matmul(sample_datas[i], V[:, :pca_reduction_dim])
        # get the center of every element of sample_datas
        # convert the tensor to numpy
        reduction_data_np = reduction_data.numpy()
...
def test(lr:float = 0.005, net:str = 'CNN_small',repair_radius:float = 0.1, repair_number = 200, refine_top_k = 300,
    ...
    test_defaults = {
        ...
        'loose_bound': 3, # CNN_small: r: 0.05,0.1;0.1,0.2;0.3,0.6; 
        'sample_amount': 1000,
        'no_pts': False,
        'no_refine': False,
        'debug': False,
    ...
    for net in ['CNN_small']:
        ...
        # for patch_size in ['big']:
            ...
            # for radius in [0.05,0.1,0.3]: #,0.1,0.3
                for repair_number,test_number in zip([20],[200]):
