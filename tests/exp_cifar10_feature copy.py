...
def repair_cifar(args: Namespace, weight_clamp = False)-> Tuple[int, float, bool, float]:
    ...
    def get_bitmap(in_lb: Tensor, in_ub: Tensor, in_bitmap: Tensor, batch_inputs: Tensor, labels,repairlabels,predicted):
        '''
        in_lb: n_prop * input
        in_ub: n_prop * input
    ...
    sample_datas_list, data_number_record_list = sample_datas_from_radius(net, originalset, radius = args.repair_radius, datanumber = args.sample_amount)
    ...
    with torch.no_grad():
        feature_lb, feature_ub=  get_the_box_bounds_of_features(sample_features_list, feature_repairset, data_number_record_list)
...
def test(lr:float = 0.005, net:str = 'CNN_small',repair_radius:float = 0.1, repair_number = 200, refine_top_k = 300,
    ...
    test_defaults = {
        ...
        'loose_bound': 1, # 8/255: 1.5:45%, 1: 44%, 0.5:40% ,0.2:42.5%; 0.05:52%, 0.01: 52%; 
